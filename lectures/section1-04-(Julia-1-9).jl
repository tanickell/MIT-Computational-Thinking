...
# ╔═╡ 452aa6f3-e990-49ca-ad0d-3b392311baae
fill(1, r, r)
...
# ╔═╡ 2ee57f3e-cf14-4d27-9c99-ff47eea8dfa0
corgis * 2

# ╔═╡ 39249042-0165-4786-badc-3339775a85c5
corgis * 0.5
...
# ╔═╡ a2b47892-1ee2-4f72-a151-6a486b46a7e5
corgis_tim = load(download("https://assets.echomtg.com/magic/cards/cropped/180783.hq.jpg"))

# ╔═╡ f859c1e7-1fb3-4212-974c-68c717e340c9
corgis_tim * 2

# ╔═╡ 01392c8f-d4ce-409d-b852-88b6ee245c65
corgis_tim * 0.5

# ╔═╡ d0c9309a-9860-4a2b-8bc1-d8172bffadb0
@bind c_tim Slider(0:.1:3, show_value=true, default=1)

# ╔═╡ 0f73bcae-96f6-4a62-b18c-6af104504a5a
c_tim .* corgis_tim
...
# ╔═╡ 2276e69f-1fce-48db-9010-564048338708
upsidedown_corgis_tim = corgis_tim[ end:-1:1 , :]

# ╔═╡ bac8850f-0907-465e-ad29-3136ab62680c
(.5 * upsidedown_corgis_tim .+ .5 * corgis_tim) 
...
# ╔═╡ 6d8a66c0-ae2d-46bd-9f0a-99d8cc72e53c
@bind α_tim Slider(0:.01:1 , show_value=true, default = 1.0)

# ╔═╡ 84625663-24f5-495c-ad93-4b94dc23cc08
α_tim .* corgis_tim .+ (1-α_tim) .* upsidedown_corgis_tim
...
# ╔═╡ 7aae132d-ade3-42d1-8b84-13abbdad2fe1
identity_matrix = [0 0 0 ; 0 1 0 ; 0 0 0]
...
# ╔═╡ 4af17df6-215b-4d69-abde-0d2866526b2f
# Original cell above -- this one contains alterations from the lecture :D
begin
	identity = [0 0 0 ; 0 1 0 ; 0 0 0]
	edge_detect = [0 -1 0; -1 4 -1; 0 -1 0] 
	sharpen = identity .+ edge_detect  # Superposition!
	...
	# TAN
	mitstudent = 2 .* identity - box_blur
	
	kernels = [identity, edge_detect, sharpen, box_blur, ∇x, ∇y, mitstudent]
	kernel_keys =["identity", "edge_detect", "sharpen", "box_blur", "∇x", "∇y", "mitstudent"]
	selections = kernel_keys .=> kernel_keys
	kernel_matrix = Dict(kernel_keys .=> kernels)
	md"$(@bind kernel_name Select(selections))"
end
...
# ╔═╡ 4fbb1341-6479-405c-8b25-82a6191977dc
[imfilter( corgis_tim, kernelize(kernel_matrix[kernel_name])) Gray.(1.5 .* abs.(imfilter( corgis_tim, kernelize(kernel_matrix[kernel_name])))) ]
...
# ╔═╡ 2e45b77d-bb93-4c78-9033-268d198726c9
kernel
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "e7b9594bb49def26ac13781e56ee29a8b0762e5c"
